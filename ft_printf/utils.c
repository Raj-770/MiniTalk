...
/*   Updated: 2023/11/20 15:02:41 by rpambhar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_putstr(char *str, t_variables *var)
{
	...
	while (str[i])
	{
		if (putchar_printf(str[i], 1, var) == -1)
			return (-1);
		i++;
	}
	return (0);
...
int	ft_putnbr(int n, t_variables *var)
{
	...
	if (n >= 10)
	{
		if (ft_putnbr(n / 10, var) == -1)
			return (-1);
		if (putchar_printf(((n % 10) + '0'), 1, var) == -1)
			return (-1);
	}
	else
		if (putchar_printf((n + '0'), 1, var) == -1)
			return (-1);
	return (0);
}
...
int	ft_puthexa(unsigned long n, char c, t_variables *var)
{
	...
	if (putchar_printf(base[n % 16], 1, var) == -1)
		return (-1);
	return (0);
}
